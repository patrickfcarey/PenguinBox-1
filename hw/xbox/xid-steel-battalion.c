...
static const XIDDesc desc_xid_steel_battalion = {
    ...
    /* D1: derive from the structs so these can never drift again. The
     * output report is 22 (0x16) on real hardware — the previous 32 was
     * wMaxPacketSize mistakenly used as the report size, and a 32-byte
     * SET_REPORT would STALL against our own length guard. */
    .bMaxInputReportSize = sizeof(XIDSteelBattalionReport),
    .bMaxOutputReportSize = sizeof(XIDSteelBattalionOutputReport),
...
static void update_sbc_input(USBXIDSteelBattalionState *s)
{
    ...
    if (state == NULL) {
        /* R7: the port can be unbound between a guest poll and device
         * teardown. Keep the previous report rather than aborting. */
        return;
    }
    xemu_input_update_controller(state);

    s->in_state.dwButtons = (uint32_t)(state->sbc.buttons & 0xFFFFFFFF);
    /* D5: the five toggle switches are LATCHED in toggleSwitches. Their
     * momentary press bits (34-38, i.e. 0x7C once shifted) must be excluded
     * from the button contribution, or the act of pressing a switch to turn
     * it OFF reads as ON for the duration of the press — and feeding a real
     * controller's absolute switch level would latch ON permanently. */
    s->in_state.bMoreButtons =
        (uint8_t)((state->sbc.buttons >> 32) & 0x7F & ~SBC_TOGGLE_BITS_HI);
                                                   ...
                                                   uint8_t *data)
{
    ...
    case ClassInterfaceRequest | HID_GET_REPORT:
        ...
        if (value == 0x0100) { /* input */
            ...
            /* D2: guest-supplied wValue; stall, never abort. */
            p->status = USB_RET_STALL;
        }
        break;
    case ClassInterfaceOutRequest | HID_SET_REPORT:
        ...
        if (value == 0x0200) { /* output */
            /* D2/D4: guard on OUR struct size, never on out_state.length.
             * The old guard compared wLength against a length byte the guest
             * had itself written, then memcpy'd sizeof() bytes out of a buffer
             * that could be shorter (over-read), and finally asserted on the
             * guest-supplied result — aborting the emulator. The in-tree FIXME
             * already said this should stall. */
            if (length == sizeof(s->out_state)) {
    ...
    case VendorInterfaceRequest | USB_REQ_GET_DESCRIPTOR:
        ...
        if (value == 0x4200) {
            /* D2: `length` is the guest's wLength. A short request must stall,
             * not abort the emulator (the old assert was guest-reachable). */
            if (length < s->xid_desc->bLength) {
                p->status = USB_RET_STALL;
                break;
            }
    ...
    default:
        ...
        p->status = USB_RET_STALL;
        /* D2: do NOT assert here — an unexpected control transfer from the
         * guest must stall the endpoint, not abort the emulator. STALL is
         * already set above. Matches the gamepad path (xid.c). */
        break;
    }
}
...
static void usb_xid_steel_battalion_handle_data(USBDevice *dev, USBPacket *p)
{
    ...
    case USB_TOKEN_IN:
        if (p->ep->nr == STEEL_BATTALION_IN_ENDPOINT_ID) {
            ...
            /* D2: unexpected endpoint — stall rather than abort. */
            p->status = USB_RET_STALL;
        }
        break;
    case USB_TOKEN_OUT:
        if (p->ep->nr == STEEL_BATTALION_OUT_ENDPOINT_ID) {
            /* D4: out_state.length is written by the GUEST (byte 1 of a
             * previous OUT), so it must never size this copy. Clamp to the
             * struct AND the packet: the former stops a short guest report
             * from tripping usb_packet_copy's iov assert. (in_state.bLength
             * is ours — set from sizeof() at realize — so the IN path above
             * is safe.) */
        ...
        break;
    default:
        /* D2: unexpected PID — stall rather than abort. */
        p->status = USB_RET_STALL;
        break;
    }
}
