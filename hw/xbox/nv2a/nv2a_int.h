...
{
    /* H-E: one guest MMIO trap into an NV2A register block. Every block
     * read/write handler funnels through these two helpers exactly once per
     * dispatch, so MMIO_DISPATCH is the total per-frame register-trap volume:
     * register polls (reads) + pushbuffer kicks (user_write). vCPU thread;
     * plain non-atomic per-frame counter. */
    nv2a_profile_inc_counter(NV2A_PROF_MMIO_DISPATCH);
...
{
    /* H-E: guest MMIO write trap — see nv2a_reg_log_read. Counted into the same
     * MMIO_DISPATCH total (reads + writes). */
    nv2a_profile_inc_counter(NV2A_PROF_MMIO_DISPATCH);
