...
{
    /* sb-graphics-research H4: per-frame 2D-blit count — the Capcom-era
     * final-composite path (grain overlay candidate). */
    nv2a_profile_inc_counter(NV2A_PROF_2D_BLIT);
    ...
    if (surf_dest) {
        if (adjusted_height < surf_dest->height ||
            ...
            /* RAM is authoritative again: reads need no interception
             * (read fast path). */
            mem_access_callback_set_read_quiesced(surf_dest->access_cb, true);
        }
     ...
     * 2D blit is the other exact NV2A_TEX writer — it knows precisely which
     * bytes it wrote ([dest_addr, clipped_dest_size)). Like the surface download
     * it contributes ONLY to Channel S: record the span and, WHEN recorded, SKIP
     * the DIRTY_MEMORY_NV2A_TEX page bitmap so page-only neighbors are not
     * falsely re-hashed, while a texture the blit genuinely overwrote still
     * byte-hits and re-hashes. If the span could not be recorded (narrowing off,
     * or span array full) note returns false and we set the page bitmap as
     * stock — always exactly one channel, never neither. DIRTY_MEMORY_VGA
     * (scanout) is always set. (SB measures 2D_BLIT=0, so this is dormant for SB
     * but keeps the split sound for blit-driven titles.) */
    if (!pgraph_vk_note_exact_dirty(d, dest_addr, clipped_dest_size)) {
        memory_region_set_client_dirty(d->vram, dest_addr, clipped_dest_size,
