...
{
    /* S0 overlap counter: per-frame max of compute descriptor-set
     * consumption, sampled at its pre-reset peak (present-wall plan S0). */
    if (r->compute.descriptor_set_index >
        g_nv2a_stats.frame_working.counters[NV2A_PROF_DESC_MAX_COMP]) {
        g_nv2a_stats.frame_working.counters[NV2A_PROF_DESC_MAX_COMP] =
