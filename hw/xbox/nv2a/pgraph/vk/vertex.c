                                        ...
                                        void *data, VkDeviceSize size)
{
    ...
    /* S0 overlap counter: bytes memcpy'd in place into the GPU-read
     * vertex-RAM mirror (KB, rounded up). This is the write the WAR
     * wait_all_slots gate protects — its per-frame volume is the audit's
     * unmeasured overlap-killer (docs/vr/overlap-resource-audit.md §2b).
     * Counted at the chokepoint, so the init-time full-VRAM upload and the
     * rare FLUSH re-upload land in their (first / flush) frames too. */
    g_nv2a_stats.frame_working.counters[NV2A_PROF_VTX_RAM_SYNC_KB] +=
        (int)((size + 1023) / 1024);
