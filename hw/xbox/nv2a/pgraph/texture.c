...
TextureShape pgraph_get_texture_shape(PGRAPHState *pg, int texture_idx)
{
    ...
    if (f.linear) {
        ...
        if (levels == 0) {
            /* MIPMAP_LEVELS == 0 is illegal — real NV2A raises a data
             * exception (HW-verified in xemu#1238). A zero here means
             * uninitialized guest memory was read as a texture format
             * (typically the 0xFF000000 MmAllocateContiguousMemory fill
             * pattern, e.g. Steel Battalion): the guest DMA'd real data
             * over a GPU surface xemu failed to invalidate first. The
             * fix for that is the DMA mem-callback routing (upstream
             * PR #2514, ported); this path is a diagnostic backstop so
             * a residual sync hole logs loudly and survives the frame
             * instead of abort()ing the session.
             */
            static unsigned int zero_level_logs;
            if (zero_level_logs < 32) {
                zero_level_logs++;
                fprintf(stderr,
                        "nv2a: texture %d MIPMAP_LEVELS==0, clamping to 1: "
                        "fmt=0x%08x ctl0=0x%08x ctl1=0x%08x raw_levels=%u "
                        "min_lod=%u max_lod=%u log_whp=%u/%u/%u dim=%u "
                        "color_fmt=0x%02x cubemap=%d tex_mode=%d "
                        "offset=0x%08x shaderprog=0x%08x\n",
                        i, fmt, ctl_0, ctl_1,
                        GET_MASK(fmt, NV_PGRAPH_TEXFMT0_MIPMAP_LEVELS),
                        min_mipmap_level, max_mipmap_level,
                        log_width, log_height, log_depth,
                        dimensionality, color_format, cubemap, tex_mode,
