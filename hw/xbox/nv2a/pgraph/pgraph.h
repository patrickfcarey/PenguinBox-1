...
typedef struct PGRAPHState {
    ...
    /* Async report delivery (VK XEMU_ASYNC_REPORTS; see vk/reports.c).
     * TRUE while an occlusion-report pending-resolve marker is armed: the
     * open batch was submitted at a report boundary and the pfifo loop must
     * poll its fence instead of parking indefinitely on fifo_cond (the
     * spin-waiting guest generates no further kicks — an untimed park with a
     * marker pending would hang it). Written and read ONLY on the pfifo
     * thread (marker arm/retire and the park decision both live there), so a
     * plain bool is race-free. Never set by the GL/null renderers or on the
     * default OFF path — the park behavior is then byte-identical. */
    bool report_marker_pending;
    /* Set (and cleared) by the SET_CONTEXT_DMA_REPORT method handler around
     * its ops.process_pending_reports() call: pending report writes must be
     * delivered to the OLD report DMA object before the remap, so the VK
     * async-reports mode upgrades that call to an unconditional blocking
     * drain instead of the pushbuffer-dry-gated poll. Ignored by GL/null and
     * by the OFF path (both keep today's dry-gated behavior). pfifo thread
     * only (method handlers and the report poll run there). */
    bool force_report_drain;
