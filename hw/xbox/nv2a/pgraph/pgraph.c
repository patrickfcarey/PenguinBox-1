...
#ifdef XBOX
    /* Frame boundary: periodically ask the vCPU thread to flush the guest-PC
     * profiler ring (no-op unless XEMU_GUEST_PROFILE is set). */
    xemu_guest_profile_flip();
#endif
...
{
    /* Every pending report write must land in the OLD report DMA object —
     * pgraph_write_zpass_pixel_cnt_report maps pg->dma_report at write time,
     * so a deferred write surviving this remap would corrupt the new object.
     * The flag upgrades the poll below to an unconditional blocking drain in
     * the VK async-reports mode (XEMU_ASYNC_REPORTS); GL and the default OFF
     * path ignore it and keep today's pushbuffer-dry-gated behavior. */
    pg->force_report_drain = true;
    d->pgraph.renderer->ops.process_pending_reports(d);
    pg->force_report_drain = false;
...
/* XEMU_ELEMENT_BATCH getenv-once gate. -1 = unprobed, 0 = off, 1 = on. Read
 * only on the pfifo thread (pgraph_method dispatch), so a plain file-static
 * cache is sufficient — no cross-thread probe race like the phase timers.
 * XEMU_NO_ELEMENT_BATCH=1 is the kill-switch: it wins over XEMU_ELEMENT_BATCH
 * so a session can force the lever off without editing the environment that
 * opted in. Default (neither set) is OFF, silently. */
static int nv2a_element_batch_state = -1;

static bool nv2a_element_batch_on(void)
{
    if (nv2a_element_batch_state < 0) {
        if (getenv("XEMU_NO_ELEMENT_BATCH")) {
            nv2a_element_batch_state = 0;
            if (getenv("XEMU_ELEMENT_BATCH")) {
                fprintf(stderr,
                        "nv2a: XEMU_NO_ELEMENT_BATCH overrides "
                        "XEMU_ELEMENT_BATCH — ARRAY_ELEMENT batching off\n");
            }
        } else {
            nv2a_element_batch_state = getenv("XEMU_ELEMENT_BATCH") ? 1 : 0;
            if (nv2a_element_batch_state) {
                fprintf(stderr,
                        "nv2a: XEMU_ELEMENT_BATCH on — "
                        "NV097_ARRAY_ELEMENT16/32 index runs consumed in bulk "
                        "(INLINE_BATCH sibling)\n");
            }
        }
    }
    return nv2a_element_batch_state == 1;
}

/* Hand-expansions of DEF_METHOD_NON_INC(NV097, ARRAY_ELEMENT16/32) so the
 * dispatch wrappers can choose, per call, between the default per-word path
 * and a batched bulk-consume fast path — the sibling of the XEMU_INLINE_BATCH
 * port below (upstream PR #2930 shape) for the method Steel Battalion's D3D8
 * path actually uses: SB feeds geometry as index runs through ARRAY_ELEMENT16
 * (INLINE_ELEMENTS hot, ~1162/frame) while INLINE_ARRAY never fires, so the
 * ported INLINE_BATCH lever never engages (2026-07-26 interleaved matrix).
 * Target bucket: pfifo2 method_resid (per-word method dispatch overhead).
 *
 * Default (XEMU_ELEMENT_BATCH unset): byte-identical to the macro-generated
 * NON_INC wrappers — they delegate to pgraph_method_non_inc() with the same
 * _int handlers, so emulation state and trace output are unchanged.
 *
 * Batched (XEMU_ELEMENT_BATCH=1, gate above): the loop body is the per-word
 * _int handler body VERBATIM — same begin/end check, same draw-arrays
 * expansion branch, same capacity assert, same append order — and the loop
 * around it is pgraph_method_non_inc()'s loop minus the per-word indirect
 * handler call and the per-word pgraph_method_log(). That log call is
 * trace/debug bookkeeping only; for ARRAY_ELEMENT16 it is just the
 * run-abbreviation counter, so with batching the abbreviated trace line
 * counts dispatches instead of words — no emulation effect. Correctness
 * notes, in order of subtlety:
 *
 *  - ARRAY_ELEMENT16 packs TWO 16-bit indices per 32-bit word, appended low
 *    half first, high half second. The per-word handler appends BOTH halves
 *    of EVERY word unconditionally — there is no odd-count padding or
 *    terminator semantic at this layer (a guest wanting an odd index count
 *    ends the run via ARRAY_ELEMENT32 or sends a duplicate index; either way
 *    it arrives as ordinary words). The batch loop preserves exactly that:
 *    pairing is per-word, never across words, so a run split across pusher
 *    iterations cannot split a pair.
 *  - The draw-arrays expansion is the one stateful interruption point the
 *    per-word path has. It stays INSIDE the loop, per word: it can fire at
 *    most once per run (pgraph_expand_draw_arrays() ends in
 *    pgraph_reset_draw_arrays(), clearing its own trigger), on the first
 *    word, exactly as the per-word path ordered it.
 *  - The capacity assert stays per word, so an overflowing run aborts on the
 *    same word in both paths (method_count <= 2047 words per run and the
 *    END-flush reset make overflow unreachable in practice, but the crash
 *    point is part of bit-identity).
 *  - The pusher guarantees num_words_available >= 1 (method_count > 0 and
 *    dma_get != dma_put) and caps it at MIN(method_count, words to dma_put),
 *    so a partially-written run arrives as multiple calls in BOTH paths;
 *    consuming exactly num_words_available — the same count
 *    pgraph_method_non_inc() reports — keeps the METHOD_COUNT / DMA_GET /
 *    DCOUNT advance identical. Jumps/calls/returns are command words outside
 *    any method_count run, so a data run never wraps mid-batch.
 *  - inc==true (INC-typed writes to these registers) bypasses the batch and
 *    takes the untouched pgraph_method_non_inc() single-word path. */
DEF_METHOD_INT(NV097, ARRAY_ELEMENT16);

DEF_METHOD(NV097, ARRAY_ELEMENT16)
{
    if (nv2a_element_batch_on() && !inc) {
        for (size_t i = 0; i < num_words_available; i++) {
            parameter = ldl_le_p(parameters + i);
        ...
        *num_words_consumed = num_words_available;
        nv2a_profile_inc_counter(NV2A_PROF_ELEMENT_BATCH);
        return;
    }

    pgraph_method_non_inc(METHOD_FUNC_NAME_INT(NV097, ARRAY_ELEMENT16),
                          METHOD_HANDLER_ARGS);
}
...
/* ARRAY_ELEMENT32: the 1:1 case — one 32-bit index per word, no pairing.
 * Same hand-expansion shape and same correctness argument as ARRAY_ELEMENT16
 * above, minus the pair-packing concern. */
DEF_METHOD_INT(NV097, ARRAY_ELEMENT32);
...
DEF_METHOD(NV097, ARRAY_ELEMENT32)
{
    ...
    pgraph_method_non_inc(METHOD_FUNC_NAME_INT(NV097, ARRAY_ELEMENT32),
                          METHOD_HANDLER_ARGS);
}
...
/* XEMU_INLINE_BATCH getenv-once gate. -1 = unprobed, 0 = off, 1 = on. Read
 * only on the pfifo thread (pgraph_method dispatch), so a plain file-static
 * cache is sufficient — no cross-thread probe race like the phase timers. */
static int nv2a_inline_batch_state = -1;

static bool nv2a_inline_batch_on(void)
{
    if (nv2a_inline_batch_state < 0) {
        nv2a_inline_batch_state = getenv("XEMU_INLINE_BATCH") ? 1 : 0;
        if (nv2a_inline_batch_state) {
            fprintf(stderr, "nv2a: XEMU_INLINE_BATCH on — NV097_INLINE_ARRAY "
                            "vertex data consumed in bulk (upstream PR #2930)\n");
        }
    }
    return nv2a_inline_batch_state == 1;
}

/* Hand-expansion of DEF_METHOD_NON_INC(NV097, INLINE_ARRAY) so the dispatch
 * wrapper can choose, per call, between the default per-word path and a batched
 * bulk-consume fast path.
 *
 * Default (XEMU_INLINE_BATCH unset): byte-identical to the macro-generated
 * NON_INC wrapper — it delegates to pgraph_method_non_inc() with the same _int
 * handler, so emulation state and trace output are unchanged.
 *
 * Batched (XEMU_INLINE_BATCH=1): port of upstream xemu PR #2930
 * (xemu-project/xemu@e53fd6f4, "nv2a: consume INLINE_ARRAY vertex data in
 * bulk"). The NON_INC wrapper otherwise re-enters this handler once per 32-bit
 * word — an indirect call + pgraph_method_log() + begin/end check per word. For
 * immediate-mode D3D8 titles (Steel Battalion pushes heavy inline vertex data
 * per frame) that per-word dispatch is several ms/frame of pfifo method self-
 * time (pfifo2 method_resid). The batched path reads the whole available run in
 * one tight loop, storing identical words in identical order, and reports the
 * consumed count so the pusher advances the DMA get pointer in one step. The
 * leading assert guarantees at least one free slot, so count >= 1 whenever
 * num_words_available >= 1 (no zero-consume / DMA-get underflow). */
DEF_METHOD_INT(NV097, INLINE_ARRAY);

DEF_METHOD(NV097, INLINE_ARRAY)
{
    if (nv2a_inline_batch_on() && !inc) {
        pgraph_check_within_begin_end_block(pg);
        ...
        *num_words_consumed = count;
        nv2a_profile_inc_counter(NV2A_PROF_INLINE_BATCH);
        return;
    }

    pgraph_method_non_inc(METHOD_FUNC_NAME_INT(NV097, INLINE_ARRAY),
                          METHOD_HANDLER_ARGS);
}
