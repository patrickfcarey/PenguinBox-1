...
void nv2a_profile_flip_stall(void)
{
    ...
    /* sb-graphics-research H6: every 60 frames, one stderr line of mspf +
     * all non-zero counters from the just-completed frame — flat rig
     * runs become self-logging (counter-signature diagnosis from a plain
     * log, no HUD needed). */
    if ((g_nv2a_stats.frame_count % 60) == 0) {
        /* Sized so the S0 overlap-counter tail (the ARENA/DESC/OVERLAP
         * group, appended last in the XMAC list) cannot be silently dropped
         * by the n < size-48 append guard on a busy Steel Battalion line —
         * a truncated detector reads as a zero (M-3). */
        char buf[1536];
        int n = snprintf(buf, sizeof(buf), "nv2a-prof: mspf=%d",
                         g_nv2a_stats.frame_working.mspf);
        for (unsigned int i = 0; i < NV2A_PROF__COUNT; i++) {
            int v = g_nv2a_stats.frame_working.counters[i];
            if (v && n < (int)sizeof(buf) - 48) {
                n += snprintf(buf + n, sizeof(buf) - n, " %s=%d",
                              nv2a_profile_get_counter_name(i), v);
            }
        }
        fprintf(stderr, "%s\n", buf);
    }
