...
void *pfifo_thread(void *arg)
{
    ...
    while (true) {
        ...
        if (!d->pfifo.halt) {
            /* pfifo: pushbuffer/method dispatch SELF time. method_end
             * attributes the pusher span MINUS whatever leaf children ran
             * inside it (a draw's texhash / surface upload-download / mid-draw
             * submit, plus the FLIP present) to the `method` bucket, isolating
             * the pure guest-method consumption cost of the ~2037 draws. The
             * inline gate keeps the OFF path a single atomic read + branch (no
             * function call), matching the file's other hot brackets. */
            bool pt = nv2a_phase_timers_on();
            int64_t pt_method = pt ? nv2a_phase_method_begin() : 0;
            pfifo_run_pusher(d);
            if (pt) {
                nv2a_phase_method_end(pt_method);
            }
        }

        /* pfifo: occlusion-query drain — the STALLED finish + result resolve
         * (or the async submit+wait). Loop-level, so it never nests in the
         * pusher and is a clean leaf of its own. */
        {
            NV2A_PHASE_LEAF(PF_LEAF_QUERIES);
        ...
        if (!d->pfifo.fifo_kick) {
            ...
            /* Phase 0: pfifo idle. Everything else in this loop is pfifo
             * "work"; pfifo_work is derived as wall - pfifo_idle at the flip. */
            bool pt = nv2a_phase_timers_on();
            int64_t pt_t0 = pt ? nv2a_phase_now_ns() : 0;
            if (d->pgraph.report_marker_pending) {
                /* Async report delivery (XEMU_ASYNC_REPORTS): a pending-
                 * resolve marker is armed and the guest is spin-reading
                 * report RAM — it will never kick this condition variable.
                 * Park with a ~1 ms timeout so the loop resurfaces to run the
                 * fence poll (pgraph_process_pending_reports above); the
                 * indefinite park below would deadlock the guest's spin.
                 * Written and read only on this thread (see pgraph.h), so no
                 * atomics are needed for the decision. */
                qemu_cond_timedwait(&d->pfifo.fifo_cond, &d->pfifo.lock, 1);
            } else {
                /* The untimed park must never be entered with a marker
                 * pending — tautological given the branch above, but armed
                 * against future re-orderings of this loop (Lane 2 SS4.3
                 * failure mode 1: park-with-pending hang). */
                assert(!d->pgraph.report_marker_pending);
                qemu_cond_wait(&d->pfifo.fifo_cond, &d->pfifo.lock);
            }
            if (pt) {
                nv2a_phase_add_pfifo_idle(pt_t0);
            }
        }
