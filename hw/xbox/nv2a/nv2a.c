...
static void nv2a_lock_fifo(NV2AState *d)
{
    ...
    bql_unlock();
    /* Phase 0: "other vCPU blocked" — the guest (via an MMIO register access
     * that needs the pfifo quiesced) sleeps until the pfifo thread goes
     * idle. Folds into `blocked` but NOT `dl_stall`, so a fat blocked-minus-
     * dl_stall gap surfaces MMIO-quiesce cost (roadmap term f). Bracket only
     * the wait. Steady-state callers are the vCPU thread; the device-reset
     * caller is not per-frame (racy-by-a-frame acceptable). */
    bool pt = nv2a_phase_timers_on();
    int64_t pt_t0 = pt ? nv2a_phase_now_ns() : 0;
    qemu_cond_wait(&d->pfifo.fifo_idle_cond, &d->pfifo.lock);
    if (pt) {
        nv2a_phase_add_fifo_wait(pt_t0);
    }
    bql_lock();
