...
void xemu_input_update_sdl_kbd_controller_state(ControllerState *state)
{
    ...
    if (strcmp(bound_driver, DRIVER_STEEL_BATTALION) == 0) {
        ...
        if (kbd[sdl_sbc_kbd_scancode_map[49]])
            state->sbc.axis[SBC_AXIS_LEFT_PEDAL] = SBC_PEDAL_FULL;
        if (kbd[sdl_sbc_kbd_scancode_map[50]])
            state->sbc.axis[SBC_AXIS_RIGHT_PEDAL] = SBC_PEDAL_FULL;
        if (kbd[sdl_sbc_kbd_scancode_map[51]])
            state->sbc.axis[SBC_AXIS_MIDDLE_PEDAL] = SBC_PEDAL_FULL;
...
#define SDL_GET_AXIS(state, axis)    \
    SDL_GetGamepadAxis(              \
        ...
        /* D3: SDL triggers report 0..32767; SBC pedals are 0..0xFFFF, so a
         * fully-pulled trigger must scale to full pedal travel. */
        state->sbc.axis[SBC_AXIS_MIDDLE_PEDAL] = sbc_pedal_from_trigger(
            SDL_GET_AXIS(state, SDL_GAMEPAD_AXIS_LEFT_TRIGGER));
        state->sbc.axis[SBC_AXIS_RIGHT_PEDAL] = sbc_pedal_from_trigger(
...
{
    for (int i = 0; i < 4; i++) {
        if (bound_controllers[i] != NULL &&
            bound_controllers[i]->type == INPUT_DEVICE_SDL_KEYBOARD &&
            strcmp(bound_drivers[i], DRIVER_STEEL_BATTALION) == 0) {
            return bound_controllers[i];
        }
    }
    return NULL;
}
...
{
    return xemu_input_get_keyboard_sbc() != NULL;
}
