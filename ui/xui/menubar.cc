...
void ProcessKeyboardShortcuts(void)
{
    ...
    /* With the keyboard driving a Steel Battalion controller, F10-F12
     * belong to the cockpit (fuel / buffer material / VT location —
     * ISS-B12); the actions stay reachable via the menus. */
    bool sbc_kbd = xemu_input_keyboard_feeds_sbc();

    if (!sbc_kbd && ImGui::IsKeyPressed(ImGuiKey_F12)) {
        ActionScreenshot();
    }

    if (!sbc_kbd && ImGui::IsKeyPressed(ImGuiKey_F11)) {
        xemu_toggle_fullscreen();
    }

#ifdef CONFIG_RENDERDOC
    if (!sbc_kbd && ImGui::IsKeyPressed(ImGuiKey_F10) &&
        nv2a_dbg_renderdoc_available()) {
...
#ifdef CONFIG_VULKAN
            /* Recenter VR View: re-anchors the world-locked screen (and the
             * forthcoming cockpit panel) to the current head pose. Mirrors
             * the always-on Ctrl+F8 chord in ui/xemu.c, so it clears the SBC
             * cockpit gate too (ISS-B12). A no-op when VR is off, so it
             * stays enabled unconditionally rather than probing session
             * state. */
            if (ImGui::MenuItem("Recenter VR View", "Ctrl+F8")) {
                xemu_vr_request_recenter();
            }
#endif

            /* Only meaningful while the keyboard drives the SBC; greyed out
             * otherwise so the entry explains itself rather than vanishing. */
            if (ImGui::MenuItem("SBC Cockpit Overlay", "F3",
