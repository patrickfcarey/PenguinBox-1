...
#ifdef CONFIG_VULKAN
    SectionTitle(xemu_vr_active() ? "Virtual Reality — session active" :
                                    "Virtual Reality");
    Toggle("Enable VR", &g_config.vr.enable,
           "Head-tracked virtual screen via OpenXR (Vulkan renderer only). "
           "Applies at next launch; the rig launcher manages this "
           "(XEMU_VR=1). F8 recenters the screen.");
    /* Screen geometry sliders write straight into g_config.vr.* — the VR
     * compositor reads them every frame, so they tune LIVE in-headset. */
    float t;
    t = (g_config.vr.screen_distance - 0.5f) / 4.5f;
    Slider("Screen distance", &t, "0.5 - 5 m (live)");
    g_config.vr.screen_distance = 0.5f + t * 4.5f;

    t = (g_config.vr.screen_height - 0.5f) / 2.5f;
    Slider("Screen height", &t, "0.5 - 3 m (live)");
    g_config.vr.screen_height = 0.5f + t * 2.5f;

    t = g_config.vr.screen_arc / 180.0f;
    Slider("Screen curve", &t, "0 = flat quad; degrees of cylinder arc "
                               "(needs runtime cylinder-layer support)");
    g_config.vr.screen_arc = t * 180.0f;

    t = (g_config.vr.screen_vertical_offset + 1.0f) / 2.0f;
    Slider("Screen vertical offset", &t, "-1 - +1 m vs the recenter anchor");
    g_config.vr.screen_vertical_offset = -1.0f + t * 2.0f;
#endif
