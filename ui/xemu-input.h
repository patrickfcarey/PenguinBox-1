...
/* D5: the five latching toggle switches, as they appear in bMoreButtons
 * (bits 34-38 >> 32). Their momentary press bits must be masked out of the
 * report; only the latched state in toggleSwitches is reported. */
#define SBC_TOGGLE_BITS_HI 0x7C

/* D3: SBC pedals are UNSIGNED 0..0xFFFF on the wire, but share the signed
 * axis[] array; the report builder bit-casts to uint16_t. Full travel is
 * therefore 0xFFFF. Writing 32767 (INT16_MAX) yields 0x7FFF, which the game
 * reads as only ~50% depression. */
#define SBC_PEDAL_FULL ((int16_t)0xFFFF)

/* Scale an SDL trigger (0..32767) to full SBC pedal travel (0..0xFFFF).
 * A function, not a macro: the argument is typically an SDL_GetGamepadAxis()
 * call, and a macro would evaluate it twice. The (v<<1)|(v>>14) form reaches
 * exactly 0xFFFF at full pull, where a plain *2 would stop one short. */
static inline int16_t sbc_pedal_from_trigger(int16_t v)
{
    unsigned t = (v < 0) ? 0u : (unsigned)v;
    return (int16_t)(uint16_t)((t << 1) | (t >> 14));
}
...
/* True when a bound port runs the Steel Battalion controller driver fed by
 * the keyboard — the SBC map claims F5-F12 etc., so global F-key hotkeys
 * must stand down (ISS-B12). */
bool xemu_input_keyboard_feeds_sbc(void);

/* The keyboard-fed Steel Battalion controller, or NULL. Same predicate as
 * xemu_input_keyboard_feeds_sbc() (which is now a wrapper), but hands back the
 * state so a read-only consumer — the cockpit overlay — can mirror it. */
ControllerState *xemu_input_get_keyboard_sbc(void);
