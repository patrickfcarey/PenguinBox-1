...
#ifdef XBOX
    /* PenguinBox per-game hack: intercept the Steel Battalion radar-scan
     * breakpoints here, BEFORE the guest-#DB injection path below.  When it
     * matches one of our scanner EIPs it resumes the guest via
     * cpu_loop_exit_noexc() and does not return; otherwise it is a no-op. */
    xbox_game_hacks_maybe_skip_scanner(cs);
#endif
