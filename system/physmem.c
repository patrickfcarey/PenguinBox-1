...
#ifdef XBOX
/* Phase-0 emulation-overhead profiler counters (nv2a_profile_inc_counter).
 * Resolved via -iquote <source-root>; debug.h includes only <stdint.h>, so
 * there is no include cycle back into the memory subsystem. */
...
/*
 * XEMU_TLB_FASTPATH read fast path master toggle (default OFF).
 *
 * When ON, clean (read-quiesced) GPU-surface pages are filled without a read
 * watchpoint so guest loads take the full TCG fast path, and quiesce/dirty
 * transitions reconcile the affected TLB pages (see
 * mem_access_callback_update_read_arming). XEMU_NO_TLB_FASTPATH forces it off
 * and wins over XEMU_TLB_FASTPATH. Evaluated once and cached; getenv reads a
 * concurrently-unmodified environment, so the first-call race between the
 * pfifo and vCPU threads is benign — both compute the same value.
 */
bool xemu_tlb_fastpath_enabled(void)
{
    static int cached = -1;
    int v = qatomic_read(&cached);

    if (v < 0) {
        v = (getenv("XEMU_TLB_FASTPATH") != NULL &&
             getenv("XEMU_NO_TLB_FASTPATH") == NULL) ? 1 : 0;
...
/*
 * Read-aware variant used by the read fast path at TLB fill time. WRITES still
 * arm unconditionally (BP_MEM_WRITE on any overlap — writes must always
 * dispatch: upload_pending marking, DMA-write detection), but the READ arm
 * (BP_MEM_READ) is suppressed for quiesced callbacks, whose RAM copy is already
 * coherent; their pages then fill with no read watchpoint and guest loads skip
 * the trap entirely. The plain matcher above is retained for the write path and
 * for the toggle-off case, keeping default behavior byte-for-byte unchanged.
 */
int mem_access_callback_address_matches_for_read(CPUState *cpu, hwaddr addr,
                                                 hwaddr len)
{
    int ret = 0;
    MemAccessCallback *cb;
    ...
    QTAILQ_FOREACH(cb, &cpu->mem_access_callbacks, entry) {
        if (access_callback_address_matches(cb, addr, len)) {
            ret |= BP_MEM_WRITE;
            if (!qatomic_read(&cb->read_quiesced)) {
                ret |= BP_MEM_READ;
            }
        }
    }
...
/*
 * Owner-facing setter for a callback's read_quiesced state (called from the
 * pfifo thread at the surface truth transitions: registration, download
 * complete, blit discard, set_surface_dirty).
 *
 * Toggle OFF: preserve the shipped dispatch-skip fast path exactly — set the
 * flag only. Fill-time arming stays unconditional, so no fast read entries ever
 * exist and there is nothing to reconcile.
 *
 * Toggle ON: on an actual state change, reconcile the callback's TLB pages so
 * the next fill re-evaluates read arming — arm on un-quiesce (drop stale fast
 * entries so post-draw reads trap again) and drop the trap on quiesce. The flag
 * is published BEFORE the (async) reconcile so any fill during the un-synced
 * window already uses the new arming, and any still-armed trapped read in that
 * window is covered by the dispatch-skip in mem_check_access_callback_ramaddr.
 * No-op sets (e.g. redraw of an already-dirty surface) skip the walk.
 */
void mem_access_callback_set_read_quiesced(MemAccessCallback *cb, bool quiesced)
{
    if (!cb) {
        return;
    }
    if (!xemu_tlb_fastpath_enabled()) {
        qatomic_set(&cb->read_quiesced, quiesced);
        return;
    }
    if (qatomic_read(&cb->read_quiesced) != quiesced) {
        qatomic_set(&cb->read_quiesced, quiesced);
        mem_access_callback_update_read_arming(qemu_get_cpu(0), cb);
    }
}
                                              ...
                                              void *opaque)
{
    ...
    cb->func = func;
    cb->opaque = opaque;
    cb->read_quiesced = false; /* dispatch everything until the owner opts in */
    ...
    /* H-B instrumentation: a surface watch-range registration costs one FULL
     * all-CPU TLB flush. MEMCB_REGISTER = register/unregister churn volume;
     * TLB_FLUSH_ALL = the full flushes it forces. Incremented on the caller
     * (pgraph/pfifo surface-management) thread as a plain non-atomic per-frame
     * counter — same vCPU/pfifo tolerance as NV2A_PROF_SURF_CPU_READ. */
    nv2a_profile_inc_counter(NV2A_PROF_MEMCB_REGISTER);
    nv2a_profile_inc_counter(NV2A_PROF_TLB_FLUSH_ALL);
...
void mem_access_callback_remove_by_ref(CPUState *cpu, MemAccessCallback *cb)
{
    ...
    /* H-B: unregister path — same full-flush cost as insert above. */
    nv2a_profile_inc_counter(NV2A_PROF_MEMCB_REGISTER);
    nv2a_profile_inc_counter(NV2A_PROF_TLB_FLUSH_ALL);
}
                                       ...
                                       hwaddr ram_addr, vaddr len, int flags)
{
    ...
    QTAILQ_FOREACH(cb, &cpu->mem_access_callbacks, entry) {
        /* Read fast path: a quiesced callback's readable state is already
         * coherent in RAM — skip dispatch (mutex + overlap walks in the
         * consumer) entirely. Writes always dispatch. */
        if (!is_write && qatomic_read(&cb->read_quiesced)) {
            continue;
        }
        if (access_callback_address_matches(cb, ram_addr, len)) {
            ...
            /* Post-fast-path dispatch volume: a callback actually fired. Reads
             * to a quiesced callback were already skipped above, so
             * READ_DISPATCH should sit near 0 (confirms the read fast path);
             * WRITE_DISPATCH is the per-frame write-intercept load. vCPU-thread
             * increment, one level above the SURF_CPU_* counters in cb->func. */
            nv2a_profile_inc_counter(is_write ? NV2A_PROF_MEMCB_WRITE_DISPATCH
                                              : NV2A_PROF_MEMCB_READ_DISPATCH);
...
#ifdef XBOX
/*
 * Read fast path reconcile (XEMU_TLB_FASTPATH). After a callback's
 * read_quiesced state changes, invalidate its TLB pages so the next fill
 * re-evaluates read arming. cb->addr is a ram_addr_t but the TLB is keyed by
 * guest vaddr, so — like tlb_reset_dirty_range_all above — translate the
 * surface's ram_addr range to a host range and hand it to the host-matched
 * invalidation walk. Called from the pfifo thread; the walk itself is queued
 * onto the target vCPU.
 */
void mem_access_callback_update_read_arming(CPUState *cpu, MemAccessCallback *cb)
{
    ...
    start = cb->addr & TARGET_PAGE_MASK;
    end = TARGET_PAGE_ALIGN(cb->addr + cb->len);

    RCU_READ_LOCK_GUARD();
    block = qemu_get_ram_block(start);
    if (block != qemu_get_ram_block(end - 1)) {
        /* Surface not resident in a single RAMBlock (never happens for a
         * VRAM-backed surface). Fall back to a correct-but-coarse full flush. */
        tlb_flush_all_cpus_synced(cpu);
        /* H-B: rare coarse-fallback full flush from the read-arming reconcile.
         * Normally 0 — if TLB_FLUSH_ALL exceeds MEMCB_REGISTER, this fired. */
        nv2a_profile_inc_counter(NV2A_PROF_TLB_FLUSH_ALL);
        return;
    }
    host_start = (uintptr_t)ramblock_ptr(block, start - block->offset);
    tlb_invalidate_ram_range_by_ref(cpu, host_start, end - start);
}
#endif /* XBOX */
