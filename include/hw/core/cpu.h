...
typedef struct MemAccessCallback {
    ...
    void *opaque;
    /* When true, READ accesses in range skip dispatch entirely — the fast
     * path for consumers whose readable state is already coherent (e.g. a
     * GPU surface whose RAM copy is up to date). Writes always dispatch.
     * Owner toggles it with mem_access_callback_set_read_quiesced(); a
     * racing read at the transition sees at worst the pre-transition
     * behavior, which is the emulation-order ambiguity that already
     * exists between the guest CPU and the GPU. */
    bool read_quiesced;
    QTAILQ_ENTRY(MemAccessCallback) entry;
} MemAccessCallback;

/* Toggle a callback's read_quiesced state (see MemAccessCallback.read_quiesced
 * and mem_access_callback_set_read_quiesced() in system/physmem.c). Under the
 * XEMU_TLB_FASTPATH toggle a real state change also reconciles the callback's
 * TLB pages; with the toggle off it is just the flag store. */
...
/* XEMU_TLB_FASTPATH read fast path (default OFF). */
bool xemu_tlb_fastpath_enabled(void);
/* Read-aware match: WRITE arm unconditional, READ arm only for non-quiesced
 * callbacks — used at TLB fill time when the fast path is enabled. */
int mem_access_callback_address_matches_for_read(CPUState *cpu, hwaddr addr,
                                                 hwaddr len);
/* Reconcile a callback's TLB pages after its read_quiesced state changed. */
