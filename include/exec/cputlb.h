...
#ifdef XBOX
/*
 * XEMU_TLB_FASTPATH read fast path support.
 *
 * Invalidate any TLB entry backing host range [host_start, host_start+length)
 * so that it re-fills with fresh XBOX mem-access-callback read arming. Matched
 * by HOST address (mirrors tlb_reset_dirty()) because the caller is a GPU
 * surface quiesce transition running on the pfifo thread, which holds only the
 * surface's ram_addr and has no guest virtual address to flush. The work is
 * queued to run on @cpu (the target vCPU); invalidation is always safe, so an
 * over-broad match at worst forces a harmless re-fill.
 */
