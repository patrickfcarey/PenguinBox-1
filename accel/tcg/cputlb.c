 ...
 * there is no include cycle back into the TCG core. */
...
#ifdef XBOX
/*
 * XEMU_TLB_FASTPATH read fast path — TLB reconcile for mem-access-callback
 * quiesce transitions.
 *
 * When a GPU surface's RAM copy becomes coherent (download complete / blit
 * discard) its callback is read-quiesced and its clean pages fill WITHOUT a
 * read watchpoint, so guest loads take the full TCG fast path. When the GPU
 * draws again the callback un-quiesces and those pages must re-arm. Both
 * transitions run on the pfifo thread, which has only the surface's ram_addr
 * and no guest virtual address; there is no reverse ram_addr -> vaddr index,
 * so — exactly like tlb_reset_dirty() — we walk every installed entry and
 * match by HOST address: host = (comparator & TARGET_PAGE_MASK) + addend.
 *
 * Invalidation direction only ever COSTS a re-fill, never correctness: an
 * over-broad match harmlessly re-fills an unrelated page, whereas MISSING an
 * entry would leave a stale fast entry (a silent stale read — the failure we
 * must avoid), so we invalidate any non-empty entry whose host page is in
 * range regardless of its current arming.
 */
typedef struct XboxTLBHostRange {
    uintptr_t host_start;
    uintptr_t length;
} XboxTLBHostRange;

static inline bool xbox_tlb_entry_invalidate_hostrange(CPUTLBEntry *ent,
                                                       uintptr_t start,
                                                       uintptr_t length)
{
    uintptr_t comp, host;
    ...
    /*
     * Every valid comparator of an entry encodes the same guest page and
     * shares one addend; use whichever permission is present (read arming is
     * about loads, so prefer addr_read). Flag bits live below TARGET_PAGE_BITS
     * and are masked off — identical host math to tlb_reset_dirty_range_locked.
     */
    comp = ent->addr_read;
    if (comp == (uintptr_t)-1) {
        comp = ent->addr_write;
    ...
    host = (comp & TARGET_PAGE_MASK) + ent->addend;
    if ((host - start) < length) {
...
static void xbox_tlb_invalidate_ram_range_async(CPUState *cpu,
                                                run_on_cpu_data data)
{
    XboxTLBHostRange *r = data.host_ptr;
    uintptr_t start = r->host_start;
    uintptr_t length = r->length;
    int mmu_idx;

    g_free(r);
    /* memset-invalidation of another running vCPU's TLB is unsafe; this must
     * be the target vCPU's own thread (guaranteed by async_run_on_cpu). */
    assert_cpu_is_self(cpu);
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        ...
        /* Most of NB_MMU_MODES carry nothing on i386 — skip empty tables. */
        if (desc->n_used_entries == 0) {
            continue;
        }
        n = tlb_n_entries(fast);
        for (i = 0; i < n; i++) {
            if (xbox_tlb_entry_invalidate_hostrange(&fast->table[i],
                                                    start, length)) {
                tlb_n_used_entries_dec(cpu, mmu_idx);
            }
        }
        for (i = 0; i < CPU_VTLB_SIZE; i++) {
            if (xbox_tlb_entry_invalidate_hostrange(&desc->vtable[i],
                                                    start, length)) {
                tlb_n_used_entries_dec(cpu, mmu_idx);
            }
        }
    ...
    /*
     * No tb_jmp_cache flush: mem-access-callback ranges are GPU surfaces
     * (data), never executed as code, and this reconcile only concerns data
     * read arming. Surface (un)bind already does a full flush for any aliasing.
     */
}

void tlb_invalidate_ram_range_by_ref(CPUState *cpu, uintptr_t host_start,
                                     uintptr_t length)
{
    XboxTLBHostRange *r;
    ...
    r = g_new(XboxTLBHostRange, 1);
    r->host_start = host_start;
    r->length = length;
    /*
     * Queue on the target vCPU. From the pfifo thread this is cross-thread
     * async work (runs at the next TB boundary); if we already are the vCPU it
     * still queues and runs after the current TB. Lazy is fine — the un-synced
     * window is the same CPU-vs-GPU emulation-order ambiguity documented at the
     * callers, and it closes well before the guest can observe the draw.
     */
    async_run_on_cpu(cpu, xbox_tlb_invalidate_ram_range_async,
                       ...
                       vaddr addr, CPUTLBEntryFull *full)
{
...
#ifdef XBOX
    /* H-B refill proxy: every TLB slow-path fill installs its entry here. After
     * a full flush (mem-callback register/unregister) the working set re-fills
     * through this path, so TLB_FILL measures the refill cost the flush storm
     * forces. vCPU thread (asserted self above); plain non-atomic counter. */
    nv2a_profile_inc_counter(NV2A_PROF_TLB_FILL);
#endif
    ...
    if (xemu_tlb_fastpath_enabled()) {
        /*
         * Read fast path: arm this page's READ watchpoint only if some
         * overlapping mem-access callback still needs to intercept reads
         * (read_quiesced == false). A page whose callbacks are all quiesced
         * fills clean and its guest loads take the full TCG fast path with no
         * trap. The WRITE arm stays unconditional (writes must always dispatch:
         * upload_pending marking, DMA-write detection). This only changes the
         * XBOX callback contribution to wp_flags; the generic gdb-watchpoint
         * bits from cpu_watchpoint_address_matches() above are untouched.
         */
        wp_flags |= mem_access_callback_address_matches_for_read(
            cpu, iotlb & TARGET_PAGE_MASK, TARGET_PAGE_SIZE);
    } else {
