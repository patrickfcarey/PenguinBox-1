...
#ifdef XBOX
    /*
     * Statistical guest-PC sampler (accel/tcg/xemu-guest-profile.c).  This is
     * the vCPU thread at a TB boundary; @pc is the block-head guest linear PC.
     * OFF cost is exactly this one never-taken branch.  When armed, chaining is
     * forced off in curr_cflags() so every TB reaches this point.
     */
    if (unlikely(xemu_gp_enabled)) {
        xemu_guest_profile_tb((uint32_t)pc);
    }
#endif
    trace_exec_tb(tb, pc);
