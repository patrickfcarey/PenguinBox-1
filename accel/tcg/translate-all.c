...
TranslationBlock *tb_gen_code(CPUState *cpu, TCGTBCPUState s)
{
    ...
    if (tb) {
        ...
        recycled = true;
#ifdef XBOX
        /* Invalidated-TB reuse: a flush/invalidation dropped this TB from the
         * live htable but its code was intact, so it is revived here instead of
         * re-translated. Pairs with TB_TRANSLATE to split TB churn that cost a
         * full re-translation from churn absorbed by cheap reuse — the H-B
         * "does the flush storm reach the TB layer" signal. vCPU thread. */
        nv2a_profile_inc_counter(NV2A_PROF_TB_RECYCLE);
#endif
        goto recycle_tb;
    }
...
#ifdef XBOX
    /* A brand-new TB was just generated. This point is past every
     * buffer-overflow / restart_translate retry and is never reached by the
     * recycle fast path above, so it counts fresh translations exactly once.
     * Re-translation storms driven by the TLB-flush churn surface here as a
     * per-frame spike; one inc per fresh TB is negligible beside the TCG
     * frontend that just ran. vCPU thread. */
    nv2a_profile_inc_counter(NV2A_PROF_TB_TRANSLATE);
#endif
