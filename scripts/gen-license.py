...
class Lib:
    ...
    def version(self):
        ...
        if self.pkgconfig:
            try:
                ...
                # No system .pc (e.g. slirp satisfied from the vendored
                # meson subproject) — fall through to other strategies.
                pass
        ...
        # Last resort (vendored/subproject deps with no system package):
        # don't fail the whole packaging step over a version label.
        self._version = "unknown (vendored)"
        return self._version
    ...
    def license_text(self):
        ...
        try:
            d = requests.get(self.license_url, timeout=30).content.decode(
                "utf-8"
            )
        except requests.exceptions.RequestException as e:
            # Offline/slow rig builds must not hang or die in packaging over
            # a license-text fetch; ship a pointer instead.
            return (
                "License text unavailable at build time (" + str(e.__class__.__name__)
                + "); see " + self.license_url
            )
